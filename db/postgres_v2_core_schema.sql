...
-- Derived strategy analysis remains immutable and traceable to settlement facts.
CREATE TABLE v2_directional_outcomes (
    ...
    net_pnl NUMERIC(38,18) NOT NULL, return_pct NUMERIC(38,18) NOT NULL,
...
CREATE INDEX v2_directional_outcomes_rollup ON v2_directional_outcomes(
    exchange,account_id,environment,product,producer,symbol,event_type,closed_at_ms
);
CREATE TRIGGER v2_directional_outcomes_immutable BEFORE UPDATE OR DELETE ON v2_directional_outcomes
FOR EACH ROW EXECUTE FUNCTION v2_reject_mutation();

CREATE TABLE v2_directional_followups (
...
CREATE TRIGGER v2_directional_followups_immutable BEFORE UPDATE OR DELETE ON v2_directional_followups
FOR EACH ROW EXECUTE FUNCTION v2_reject_mutation();
