...
新增 `ProducerPublisher` 的全批事务确认、S0/S3 计算结果适配器和
`RedisMarketContext` 的版本化快照读写。来源快照引用阻止新信号使用较旧缓存，
决策仍冻结实际读到的有效快照。PG 不保存全量行情历史，仅保存信号与批次摘要。
详见 [生产批次说明](V2_PRODUCER_PUBLICATION.md)。旧扫描循环未切换，事件生命周期
和冷却仍需迁到 PG，第 1 项保持进行中。

新增 29 项 QA；全仓 3247 passed、10 skipped、1 个既有 warning；Ruff/格式/diff
检查通过，临时实例已停止。没有修改密钥权限或使用币安测试账户下单。

## S3 持久生命周期与纯检测完整检查点

新增 S3FrameProcessor、纯失败突破状态机及 S3Runtime 组合入口；生命周期、冷却、
突破状态、帧回执、信号和生产批次同一 PG 事务提交，提交后才投影 Redis。
重启和同帧重放复用旧身份与有效期，缺行情不自动 END，END 不进入开仓评估。
真实纯检测规则已连接，不使用固定测试候选冒充生产规则。
详见 [S3 运行契约与剩余边界](V2_S3_DURABLE_RUNTIME.md)。

新增 41 项 QA；最终全仓 **3288 passed、10 skipped、1 个既有 warning**，
耗时 79.18 秒。Ruff、格式、diff 检查通过。一次性 PG/Redis 已停止，诊断保留在
`/tmp/v2-data-qa.mXETkM`。无运行库更新、现有服务重启、密钥权限变更或真实交易。
仍待实际行情采集/窗口时间校验与常驻运行接线，不能据此关闭全部上线门禁。

## 已收盘分钟输入与源确认检查点

新增 `services/v2_s3_candles.py`：原始 1440 根连续分钟 K 线严格校验、复用纯窗口
计算、原始输入摘要和来源边界、提交及投影后源确认。确认丢失可重放，无本地业务游标。
真实临时 PG/Redis 贯穿到策略证据；新帧过期/未来拒绝，旧帧重试不延长有效期。
详见 [分钟行情入口与旧模式差异](V2_S3_CANDLE_INPUT.md)。当前 source 仍为注入接口，
未实现网络采集、持久源重放/确认或常驻监督服务，也未更改生产扫描模式。
...
一次完成只读公共 HTTP、严格分钟采集、PG 并发权重预算与持久退避、待处理容量、
小时块复用/manifest、恢复优先监督循环和显式依赖工厂。整条路径不包含订单执行口。
同边界重启跳过下载，HTTP 错误不自动重试，不读取账户密钥；入队固定期限/年龄，
未发布帧遇到期限策略变化自动隔离，PG 时钟回退不能刷新限频额度。
详见 [完整组合契约及未完成部署门禁](V2_MARKET_PIPELINE.md)。

新增 52 项 QA；最终全仓 **3394 passed、10 skipped、1 个既有 warning**，97.51 秒。
Ruff/格式/diff 检查通过，诊断 `/tmp/v2-data-qa.tgiw0K`，临时服务已停止。
工厂贯穿使用真实临时 PG/Redis 和模拟 HTTP/CH 端口；ClickHouse local 另验真实 SQL。
未部署常驻服务或真实交易。实际网络/CH 集群贯穿、归档 GC、持久最低告警、性能
和其它账户/策略/保护单/对账门禁仍需完成。

## 行情持久告警与重启补发检查点

新增独立 PG 运维 outbox/回执/租约，不伪造交易意图；临时故障五分钟窗口合并，
不可变隔离审计补扫，发送失败退避，健康轮次补发。工厂默认接线并区分 QUEUED
与发送统计。复用投递算法，回归原财务路径；环境绑定视图防止测试/生产串发。
详见 [持久告警及外部监控边界](V2_OPERATIONAL_ALERTS.md)。

新增 23 项 QA；全仓 **3417 passed、10 skipped、1 个既有 warning**，113.97 秒。
Ruff/格式/diff 检查通过；临时服务已停止，诊断 `/tmp/v2-data-qa.lCbvUd`。
未发送真实 TG、读取账户密钥、更新运行数据库或重启现有服务。TG 传输、独立
存活监控及全部账户风控/策略/保护单/对账门禁仍未关闭，不是可上线版本。

## 账户级原子准入预算检查点

新增 `AccountRisk`：账户范围跨策略参考名义金额/计划数/冷却控制，配置版本 CAS，
不可变预留与同事务提交 CAS，未知订单不释放、未成交终止或结算证据后释放。
所有标准 Orders 开仓提交路径检查已配置账户；DataRuntime 接入显式新鲜参考端口，
tick 补扫释放，trace 保存原预算配置与参考证据。无 Redis/文件额度计数器。
详见 [预算语义、恢复与未完成生产接线](V2_ACCOUNT_RISK.md)。

新增 45 项 QA；最终全仓 **3462 passed、10 skipped、1 个既有 warning**，138.81 秒。
Ruff/格式/diff 检查通过；临时服务已停止，诊断 `/tmp/v2-data-qa.nsu0GE`。
没有更新运行数据库、安装服务或执行交易。可信行情参考适配、真实保证金/账户
对账与所有策略消除旁路仍未完成；第 2 项进行中，不宣称整套账户风控可上线。

## 归档风险参考与受控端口工厂检查点

新增 `ArchivedRiskReference`：PG 最新确认批次→内容寻址归档→完整窗口输入重算
核对→精确原始 close，带不可变 frame/archive/input 身份及原始期限进入账户预留。
锁内再次检查参考有效期；不接受策略请求附带价格，不以 Redis 单独证明来源。
`create_guarded_runtime` 绑定账户/环境与交易所端口，默认禁止提交，CLOSE 不依赖归档。
详见 [参考契约、成本与调度边界](V2_ARCHIVED_RISK_REFERENCE.md)。

新增 36 项 QA；全仓 **3498 passed、10 skipped、1 个既有 warning**，158.02 秒。
...
新增纯 directional 规则：多空九类事件、接管/逆势/ATR/延伸、评分及保护参数，
历史 hard/soft 过滤、R:R 与资金费率门槛。仓位采用精确十进制/分数与最终步长
向下取整，移除最低额强行抬升，约束余额风险、可用保证金与品种上限。
异常指标/缺失历史不再默认通过，事件 flow=0 保留为有效卖方数据。

新增账户绑定回放工厂接到 StrategyWorker：候选及拒绝完整写入 PG 决策证据，
并发/重试/提交后中断均复用原记录；s6-replay / s8-replay 与正式消费隔离。
回放不创建意图或订单，无交易启用开关；明确不是生产策略激活。
详见 [纯规则、回放契约及行为差异](V2_DIRECTIONAL_RULES.md)。

新增 112 项纯规则与 22 项隔离 PG 回放 QA；两组合计 134 项通过。
最终全仓 **3662 passed、10 skipped、1 个既有 warning**，182.60 秒。
Ruff/格式/diff 检查通过；临时 QA 服务已停止，诊断 `/tmp/v2-data-qa.WCenFL`。
策略级 PG 冷却/仓位/回撤状态、可信余额/历史来源、保护单与真实策略常驻接线
仍未完成；本检查点没有修改旧运行入口、运行数据库或真实交易所账户。

## PG 回撤状态与策略上下文检查点

新增账户范围的余额观测与峰值/暂停/恢复/亏损锁定状态，复用 BusinessState
及不可变 history；事务锁与 CAS 防并发覆盖，观测身份去重、原时间单调、提交
回执丢失可重放。新观测及当前读取均校验 PG 时间，历史回放不是当前放行依据。
8%/15%/2% 阈值采用精确比较，4h/6h 计时由源观测推进，零余额明确拒绝。

DrawdownContext 将 PG 同一观测的余额、系数、状态版本及证据接到持久策略回放，
缩短而不延长上下文期限，不修改调用者原快照。详见 [回撤契约与限制](V2_DRAWDOWN_STATE.md)。
新增 40 项纯状态/隔离 PG/策略证据接线 QA，定向测试通过。
最终全仓 **3702 passed、10 skipped、1 个既有 warning**，195.90 秒。
Ruff/格式/diff 检查通过；临时服务已停止，诊断 `/tmp/v2-data-qa.CszXTb`。
尚未接真实余额采集、下单事务内校验、净值/资金流调整或策略冷却；不启用交易。

## 2026-09-21：历史数据驱动的 V2 PM 退出修正

旧环境只读，修正 V2 既有监控实现的亏损反转续链与延期截止时间；反转平仓
失败或异常时不再在本轮进入时间止损重复尝试。未调整策略阈值、运行服务或数据库。
证据、样本局限与后续验证见 [历史退出分析](V2_HISTORICAL_EXIT_FINDINGS.md)。

新增 16 项参数化回归，定向 67 passed；完整隔离 QA：
