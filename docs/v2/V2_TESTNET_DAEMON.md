...
S6/S8 阶段还把调度器的原子追平快照写入同一周期证据：`source_signals`、
...
请求正文或异常消息；通知失败不会杀死监督循环。独立
[`v2_testnet_watchdog`](V2_TESTNET_WATCHDOG.md) 已补进程外监督：即使 daemon 或 PG
不可用仍能启动，检查依赖与审计心跳，并对异常及恢复做状态转换通知；它不读取
Binance 凭据，也没有任何交易端口。
...
[持久发布报告](V2_PERSISTENT_RELEASE_20260923.md)。历史任务已追平；仍需完成 Testnet
长时间运行、受支持方向信号的真实自然全链路、真实 reboot/依赖中断告警演练和
发布回滚。watchdog 代码与只读健康烟测完成不等于这些系统验收已经通过。
