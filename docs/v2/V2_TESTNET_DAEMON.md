...
签名 Testnet transport、不可变风险参考、账户回撤、S6/S8 独立历史和 TG 外层告警。
...
6. 行情采集；
7. S6/S8 正式策略调度；
8. 条件满足且开仓权限开启时，最多派发一笔新开仓并立即恢复、保护。
...
当前仍缺 V2 S0 上下文生产者。进程不会伪造中性 regime：缺 S0 时保护、退出、
结算和行情仍可运行，S6/S8 准入必然失败关闭。仍需完成 S0 迁移、账户风控策略的
显式预配置、Testnet 长时间运行、真实自然信号全链路、重启/依赖中断演练、可观测性和发布回滚。
