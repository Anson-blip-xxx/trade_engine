...
| Operator Decision Center | **PERSISTENCE IMPLEMENTED / UI PLANNED (DORMANT)** | standalone PG inbox/outbox schema + immutable models + injected transactional/CAS/lease adapter + isolated PG concurrency QA | auth/RBAC + read API/UI, then Telegram transport before mutation integration |
