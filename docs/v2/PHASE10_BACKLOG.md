...
| D3A PostgreSQL Operation Journal Foundation | **IMPLEMENTED / CLOSED (DORMANT)** | `operation_journal.model`; standalone PG schema; approved PG-stage/Redis-hot-state authority split | recovery scan + rollout QA |
| D3A PostgreSQL CAS Adapter | **IMPLEMENTED / CLOSED (DORMANT)** | `operation_journal.postgres`; injected create/read/version+owner CAS; typed UNKNOWN handling; isolated PG16 QA | recovery scan + rollout |
| D3A PostgreSQL Lease Fencing | **IMPLEMENTED / CLOSED (DORMANT)** | DB-clock claim/renew/release; monotonic version fence; unexpired-owner stage CAS; two-worker PG16 race QA | recovery discovery + extended fault/concurrency QA + rollout |
