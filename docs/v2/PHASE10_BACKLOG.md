...
| D3A PostgreSQL Operation Journal Foundation | **IMPLEMENTED / CLOSED (DORMANT)** | `operation_journal.model`; standalone PG schema; approved PG-stage/Redis-hot-state authority split | recovery decision engine + rollout QA |
| D3A PostgreSQL CAS Adapter | **IMPLEMENTED / CLOSED (DORMANT)** | `operation_journal.postgres`; injected create/read/version+owner CAS; typed UNKNOWN handling; isolated PG16 QA | recovery decision engine + rollout |
| D3A PostgreSQL Lease Fencing | **IMPLEMENTED / CLOSED (DORMANT)** | DB-clock claim/renew/release; monotonic version fence; unexpired-owner stage CAS; two-worker PG16 race QA | recovery decision engine + extended fault/concurrency QA + rollout |
| D3A PostgreSQL Recovery Claim | **IMPLEMENTED / CLOSED (DORMANT)** | bounded due batch; one-per-slot; active-slot exclusion; row + advisory locks; `SKIP LOCKED`; concurrent PG16 QA | stage decision engine + scheduler/runtime gates |
