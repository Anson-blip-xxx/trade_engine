...
| D3A PostgreSQL Operation Journal Foundation | **IMPLEMENTED / CLOSED (DORMANT)** | `operation_journal.model`; standalone PG schema; approved PG-stage/Redis-hot-state authority split | recovery executors + rollout QA |
| D3A PostgreSQL CAS Adapter | **IMPLEMENTED / CLOSED (DORMANT)** | `operation_journal.postgres`; injected create/read/version+owner CAS; typed UNKNOWN handling; isolated PG16 QA | recovery executors + rollout |
| D3A PostgreSQL Lease Fencing | **IMPLEMENTED / CLOSED (DORMANT)** | DB-clock claim/renew/release; monotonic version fence; unexpired-owner stage CAS; two-worker PG16 race QA | recovery executors + extended fault/concurrency QA + rollout |
...
| D3B Recovery Batch Coordinator Boundary | **IMPLEMENTED / CLOSED (DORMANT)** | claim validation + immutable work plans + injected D3D admission/revalidation; mutating work remains ownership-gated | D3C + mutation ownership + directive executors + scheduler/runtime wiring |
| D3D Recovery Generation Fence | **IMPLEMENTED / CLOSED (DORMANT)** | exact generation decision; injected typed reads; immutable canonical witness + second-read drift guard; OPEN/external policy gates | mutation-claim + executor composition |
| D2E/D3 Delayed-Event Fallback Policy | **PRODUCT DEFAULT APPROVED / IMPLEMENTED (DORMANT)** | stale-context rebuild; expired risk request cancellation; UNKNOWN query-only; protection escalation; expiring bound approvals; emergency close default-off | production SLO values + durable scheduler/alerts + ownership/executors |
| Operator Decision Center | **FOUNDATION IMPLEMENTED / UI PLANNED (DORMANT)** | standalone PG inbox/outbox schema + immutable state models; Telegram attention + trigger/current diff + secure expiring approval UX | injected PG adapter/concurrency QA, then auth/RBAC + read API/UI before mutation integration |
