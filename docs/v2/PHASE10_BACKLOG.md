...
| D3A PostgreSQL Operation Journal Foundation | **IMPLEMENTED / CLOSED (DORMANT)** | `operation_journal.model`; standalone PG schema; approved PG-stage/Redis-hot-state authority split | lease/recovery scan + rollout QA |
| D3A PostgreSQL CAS Adapter | **IMPLEMENTED / CLOSED (DORMANT)** | `operation_journal.postgres`; injected create/read/version+owner CAS; typed UNKNOWN handling | lease/recovery scan + real isolated PG QA + rollout |
